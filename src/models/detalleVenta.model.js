...
class DetalleVenta {
    ...
    async save() {
        ...
        const [result] = await connection.execute("INSERT INTOs detalle_venta (id_venta, id_producto, cantidad, precio, subtotal, descuento, total, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", [this.idVenta, this.idProducto, this.cantidad, this.precio, this.subtotal, this.descuento, this.total, createdAt]);
        connection.end();
